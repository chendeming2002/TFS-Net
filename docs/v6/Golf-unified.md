...
> 更新时间：2026-09-24
> 当前状态：**Golf-R4 训练完成（ep60）** — 技术修复全成功但性能未达 R2 基线（19.80 vs 20.14）

---

## 零、一句话总览

Golf 是 **Foxtrot 的轻量化修复线**：主干从 Foxtrot 的 2.57M 三分支升级到 3.69M（含 F3 FiLM/KV 投影），修复了 Foxtrot 的棋盘格伪影与结构缺陷；R2 首次突破 20 dB；R3 引入 5 项改进但暴露出**乘法双零死锁**（8.4% 参数永久冻结）；R4 系统性修复后训练完成，**技术缺陷全修复但 val 未达 R2**（19.80 vs 20.14），并暴露 **tile 边界伪影**与**泛化分裂**两大新问题。
...
| **Golf-R4 ep60** | 3.69M | 19.80 | 15.89（ep60）| 全部修复，但性能未达 R2，含 tile 伪影 |
...
## 十、R4 训练终判（2026-09-24）

### 10.1 最终指标（ep60，完整 60 epoch 训练完成）

| 指标 | ep60 | R2 基线 | 目标 | 达成 |
|------|:---:|:---:|:---:|:---:|
| **val PSNR** | **19.80** | 20.14 | ≥20.1 | ❌ **-0.34 dB** |
| **pair45 PSNR** | 15.89 | 16.06@ep40 | ≥17.0 | ❌ -1.17 dB |
| pair45 最佳 | 16.58@ep20 | 16.06 | ≥17.0 | ❌ -0.42 dB |
| SSIM | **0.745** | 0.740 | — | ✓ +0.005 |
| LPIPS | **0.322** | 0.314 | — | ≈ +0.008 |
| TCA 参数活跃 | **100%** | 91.6% | 100% | ✓ 达成 |
| temp 损失激活 | ✓ 0.02 | 0 | >0 | ✓ 达成 |
| NaN (ep31-60) | **0** | 1 | 0 | ✓ 完全消除 |

**核心假设被证伪**：R2 在 8.4% 参数"冻结"下达 20.14 dB，R4 解锁全部参数后**未释放容量**，val PSNR 反降。

### 10.2 验证历史（val_interval=10）

| Epoch | 日期 | val PSNR | pair45 | 备注 |
|:---:|------|:---:|:---:|------|
| 20 | 09-22 | 18.09 | **16.58** | **pair45 全局最佳** |
| 30 | 09-22 | 18.54 | 16.44 | NaN 前最佳，重启点 |
| 40 | 09-23 | 18.35 | 15.91 | NaN 爆发前 |
| 41-56 | 09-23 | 崩溃 | 崩溃 | **49,236 NaN**（92-94% step 失败）|
| 40 | 09-24 | 19.17 | 15.81 | 从 ep30 重启（upsample fp32 修复）|
| 50 | 09-24 | 18.90 | 16.53 | lr 降档 1e-4 后反降 |
| **60** | **09-24** | **19.80** | 15.89 | **最终** |

**泛化分裂重现**：ep20 pair45=16.58 最优，ep60 退至 15.89（-0.69 dB），与 R2 同款病症。

### 10.3 推理质量问题（tiled 伪影）

**发现**：`outputs/golf_r4_inference/pair45/` 131 帧输出存在 **tile 边界块状伪影**：
- 水平/垂直 224px stride 边界处亮度跳变 ±5~15（0-255 range）
- 非全局周期网格，而是**内容相关的块间不连续**
- 余弦窗口混合（`tiled_forward`）只能平滑，无法消除**特征域时序聚合不连续**

**根源**：训练在 256² patch，推理 tiled 1080×1920，每个 tile 独立处理 5 帧 TCA 聚合 → tile 间时序特征不连续。

### 10.4 R4 vs R2 对比

| 项 | R2 ep60 | R4 ep60 | 变化 |
|---|:---:|:---:|:---:|
| val PSNR | **20.14** | 19.80 | **-0.34** ↓ |
| pair45 (30帧) | 12.80 | 15.89 | +3.09 ↑ |
| pair45 最佳 | 16.06@ep40 | **16.58@ep20** | +0.52 ↑ |
| 参数活跃率 | 91.6% | **100%** | +8.4% ↑ |
| NaN 次数 | 1 | **0** | ✓ |

**判读**：R4 修复了所有技术缺陷，但 val 性能反降；pair45 泛化略改善但仍不如 ep20 早停点。

### 10.5 失败原因分析

#### 假设 1：修复引入了新瓶颈
- FiLM 调制表达力不足（F3 全局池化后 per-channel scale+shift 无法捕捉空间变化）
- KV 共享全时序可能引入噪声（原 R2 聚合统计量虽信息损失，但可能是有效全局先验）
- L_temp 噪声去相关过强，压制了 M 分支的合理高频恢复

#### 假设 2：R2 的"双零死锁"可能非永久死锁
- R2 若真 8.4% 参数永久冻结，不应达 20.14 dB
- **可能机制**：前几步梯度为 0，后续旁路梯度（prior/channel_mix）解锁 scale → proj_out 逐步激活
- R4 强行改非零初始化，可能破坏了"延迟激活"的隐式正则

#### 假设 3：256² patch 训练 + 全分辨率验证的分布错配
- 训练只见局部 patch，验证 tiled 推理，时序聚合在不同分布上表现退化
- 静态 val 集过拟合（539 clips × 6 次验证）
- 运动场景（pair45）欠拟合

### 10.6 核心结论

1. **R4 全部 6 项修复技术成功，但性能未达 R2 基线**
2. **NaN 修复完全有效**（ep31-60 零 NaN，upsample fp32 + conv1_max 监控）
3. **泛化分裂未解决**（val 过拟合，pair45 ep20 早停最优）
4. **tile 边界伪影**（时序聚合在 tiled 推理下不连续）
5. **"过度工程化"风险**：R2 的"缺陷"可能是隐式正则，修复后反降

**最终判断**：R4 是**技术上完整、性能上失败**的版本。需消融实验或根本性路线调整。
