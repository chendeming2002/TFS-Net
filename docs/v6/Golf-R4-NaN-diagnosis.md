...
- **1941 量级接近但不必然触发 NaN**（本地复现 ~1619 量级未溢出，max=707 有限）
- **真实触发机制可能依赖**：
  - 权重结构（若存在个别 `|w| > 34` 的权重，单项乘积 `1941 × |w|` 就能逼近 65504）
  - cuDNN 版本的累加精度模式
  - 特定 batch 的激活值分布触发最坏情况
- **定位到 `upsample.conv2`** 是实的（hook 确认），"为何溢出"是推断
...
4. **ep41 后续**（选择效应，非"权重污染"）：
   - NaN batch 被 skip → 不产生参数更新（何来污染？）
   - **存活 batch 继续正常训练** → 特征量级持续增大（训练目标就是增强表示）
   - 更多 batch 越过阈值 → 密度 4.4% → 39% → 92%
5. **ep42+ 全量崩溃**：选择效应下的持续增长，几乎所有 batch 都进入高量级区间 → 92-94% step 失败

**机制修正**：不是"权重被污染"，而是未触发 NaN 的 batch 继续推动模型变强，使更多 batch 进入危险区。
