...
### S1.5 v1 失败复盘与 v1.1 修正（2026-09-11）

**v1（纯 5 项）在 warmup ep2-3 即失败**，全程证据：

| 观测 | v1 实测 | S1 旧损失同期 |
|------|---------|--------------|
| gain 轨迹 | ep2 冲到 1.99 → **ep3 起 2.00/0.000 精确冻结 15 epoch** | 1.08±0.47（发展空间结构）|
...
**机制（已验证到参数化层）**：`gain = 0.5+1.5·sigmoid(raw)`，warmup 只有 ISPN 能提亮（需 ~3×）。旧损失的 `L_brightness_preserve` 迫使中间阶段自己变亮 → 增益只需补 ~1.2× 留在动态范围；v1 删除该锚 → 中间态保持暗（零门关闭无压力）→ 增益独自承担全部提亮 → **sigmoid 深度饱和（σ≈1 梯度≈0）→ 梯度死亡无法自愈** → 提亮负载改道 s_illum（冲到 1.95）。失败 run 存档于 `outputs/sdsd_f11_simple/`。

**v1.1 = 5 项 + 3 恢复锚 + 1 新护栏（8 项固定权重）**：

| 项 | λ | 作用 |
|----|:--:|------|
| L_brightness_preserve（恢复）| 0.5 | warmup 提亮锚——根因修复 |
| L_inter（恢复）| 0.3 | well-posed 增益约束（igrf 自证形态）|
| L_gain_sup（提权）| 0.05→0.3 | ISPN 锚点 |
| ReLU(s_illum−1)（新增）| 0.05 | 均值护栏——v1 改道失败实证 |

运行：`outputs/sdsd_f11_simple2/`（keepalive 目标 40ep）。**首个验证点 = ep2-3 的 diag gain 是否仍 2.00/0.000 冻结**（若存活即根因修复确认）。
