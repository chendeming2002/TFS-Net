...
> 执行状态（2026-09-11 10:10 更新）:
> - **S1（旧损失对照）**: 09-10 15:24 启动，ep13 处主动停止——产出 ep10 对照值 16.70/0.651 + phase1_5 跳变证据，存档 `outputs/sdsd_f11/`
> - **S1.5 v1（纯 5 项简化）**: 09-10 21:10 启动，**warmup ep2-3 失败**（增益 sigmoid 饱和 + 系统卡死，ep10 val 12.54），存档 `outputs/sdsd_f11_simple/`
> - **S1.5 v1.1（8 项含锚点）**: 09-11 09:07 启动，**运行中** `outputs/sdsd_f11_simple2/`（keepalive 目标 40ep）——**ep4 增益从饱和区恢复**（2.00→0.81/0.258，v1 同期永久冻结 2.00/0.000），pix 正常下降 0.385→0.145@ep6，系统死锁确认已破
...
### 5.1 运行史（2026-09-11 10:10 更新）

| Run | 配置 | 损失 | 时段 | 结局 |
|-----|------|------|------|------|
| S1 对照 | delta_flight11.yaml | TFSNetLoss（15 项 UW）| 09-10 15:24 → 21:0x | **ep13 主动停止**——产出 ep10=16.70 对照 + §7 分析证据，存档 `sdsd_f11/` |
| S1.5 v1 | 同上（loss 段替换）| Simple 纯 5 项 | 09-10 21:10 → 09-11 09:0x | **warmup 失败**（§S1.5 复盘），存档 `sdsd_f11_simple/` |
| **S1.5 v1.1** | delta_flight11_simple.yaml | Simple 8 项含锚 | 09-11 09:07 → | **运行中** `sdsd_f11_simple2/` |

### 5.2 当前运行配置（v1.1）

| 项 | 值 | 备注 |
|----|-----|------|
| 配置文件 | `configs/delta_flight11_simple.yaml` | loss.type: TFSNetLossSimple（8 项固定权重）|
| 输出目录 | `outputs/sdsd_f11_simple2/` | keepalive 守护（目标 40ep）|
| epochs / 验证 | 40 / 每 10ep | 对齐 v2/TBC1B 峰值位置 |
| AMP | true | 新管线组标准 |
| batch / accum | 2 / 8（等效 16）| 同 F10m5 |
| phase schedule | warmup 5 → phase1 → phase1_5 → phase2 | 同 F10m5 |
| 部署 | E-core 钉绑 + keepalive | RAPL 墙需手动 `sudo bash scripts/thermal_limit.sh` |
| 启动 | 2026-09-11 09:07，~3.0 it/s | warmup 期 |

**预计完成**：~22h → 09-12 上午出 ep40 终判。

---

## 6. 实验计划与验收

### S1（本次训练）验收 — ✅ 已执行完毕，终判 2026-09-13

| 验收项 | 指标 | 参照 | 结果 |
|--------|------|------|------|
| **V1 硬指标** | **pair45 全量 127 帧 PSNR ≥ 18** | TBC1B@40 = 15.69 / v2@49 = 15.04 | ❌ **13.51**（best/ep30，差距 4.5dB）|
| 全局对照 | res_test PSNR vs 20.02@40 | v2 基线（-0.1dB 管线混淆已知）| ❌ 18.72@30（-1.30），ep40 回落 18.29 |
| 机制观测 | conf 轨迹维持高位 + 静止/运动场景开始分化（非验收门槛）| T-BC1 前车之鉴：PSNR 与机制诊断同等重要 | 未复测（路由假说二次证伪预期内）|

> **终判归因**：T-BC1b 主干进完整模型后双指标显著低于概念模型——主干收益被整合层遮蔽（主嫌：s_illum 条件依赖未定型 A1 + 损失简化移除分支梯度路径 A2）。完整运行史/证据链/归因分析见 `experiments/rwkv_only_v3/RESULTS.md` §7。按 §8 回退路径执行：X3 消融 + S1 续训对照优先，P4/P5 优先级抬升。
...
### S1.5 v1 失败复盘与 v1.1 修正（2026-09-11，含 ep3 验证更新）

#### 动机（为什么要 v1.1）

两层动机链：
1. **为什么简化损失**（S1.5 的初衷）：概念模型以单 L1+0.2·(1−SSIM) 反超 15 项 UW 完整模型 +1.75dB；S1 运行进一步实证 phase1_5 辅助项激活即刻 pix 退化 66%（§7.3）——稀释与暗中间态偏置是真实成本
2. **为什么不能纯简化**（v1 的教训）：概念模型证据存在**覆盖盲区**——它没有 ISPN/DPE，不存在需要锚定的分阶变量。"单 pix+ssim" 直接移植到三分支形态时，分阶变量（gain_map/s_illum/中间亮度）失去物理锚点，warmup 期即发生不可逆失效。**简化损失的正确形态 = 核心简化 + 分阶锚点保留**，二者不矛盾

#### v1 失败全程证据

| 观测 | v1 实测 | S1 旧损失同期 |
|------|---------|--------------|
| gain 轨迹 | ep2 冲到 1.99 → **ep3 起 2.00/0.000 精确冻结 15 epoch** | 1.08±0.47（振荡并发展空间结构）|
| s_illum | 冲到 1.95±1.00（均值>1，softplus 无上界）| 0.85-0.95±0.05 |
| train pix | **卡死 0.240 共 10 epoch**（phase1 整段浪费）| 0.278→0.120 |
| ep10 val | **12.54**（-4.16dB）| 16.70 |

#### 解析（失效因果链）

```mermaid
flowchart TD
    A["v1 删除 L_brightness_preserve<br/>(5 项简化)"] --> B["中间阶段无人迫使变亮<br/>img_s1/img_s2 保持暗<br/>(零门关闭无压力)"]
    B --> C["提亮需求 ~3× 全部压给 gain<br/>gain = 0.5+1.5·sigmoid(raw)"]
    C --> D["sigmoid 深度饱和<br/>σ≈1 → 梯度≈0<br/>**增益头梯度死亡**"]
    D --> E["提亮负载改道 s_illum<br/>(softplus 无上界)"]
    E --> F["s_illum 冲到 1.95<br/>(均值爆炸)"]
    D --> G["train pix 卡死 0.240<br/>ep10 val 12.54"]
    F --> G
    G --> H["**系统级死锁**<br/>(不可自愈: 饱和区 ∂≈0)"]
```

与旧损失的机制对比：旧损失 warmup 期 `L_brightness_preserve@0.5`（ReLU 单边）迫使 img_s1 ≥ 0.8·input_mean——**中间阶段先变亮，增益只需补 ~1.2× 留在 sigmoid 动态范围内**。锚点不是可选项，是分阶架构的承重结构。

#### v1.1 设计（8 项固定权重，完整公式）

```
L = Charbonnier(res_t, GT) + 0.2·(1−SSIM(res_t, GT))      ← 核心（概念模型验证）
  + 0.05·ReLU(1−std(s_illum)) + 0.05·TV_edge(s_illum)      ← DPE 反塌缩（下界）
  + 0.05·ReLU(s_illum−1).mean()                            ← 新增: 均值护栏（上界，防改道）
  + 0.3·L1(gain, gt/img_curved.detach() clamp[0.5,2])       ← 提权: ISPN 锚点
  + 0.3·Charbonnier(clamp(img_s2·gain,0,1), GT)            ← 恢复: well-posed 增益约束
  + 0.5·[ReLU(0.8·input−μ(img_s1)) + ReLU(0.7·μ(img_s1)−μ(img_s2))]
                                                            ← 恢复: warmup 提亮锚（根因）
```

| 项 | λ | 性质 | 依据 |
|----|:--:|------|------|
| L_brightness_preserve | 0.5 | 恢复 | 根因修复——S1 实测该锚使 gain 保持 1.08±0.47 存活 |
| L_inter | 0.3 | 恢复 | igrf.py 注释自证的 well-posed 形态（img_s2·gain 而非裸 img_s2）|
| L_gain_sup | 0.05→0.3 | 提权 | v1 中 0.05 锚力不足（注意：暗中间态下其目标也钳在 2.0，早期与提亮同向——锚的价值在阶段变亮后接管）|
| ReLU(s_illum−1) | 0.05 | 新增 | v1 改道失败的直接对症——反塌缩只有下界没有上界 |

#### ep3-6 验证结果（2026-09-11 11:25 更新，判据修正 + 恢复确认）

| ep | v1.1 gain | v1.1 pix | v1 gain（对照）| v1 pix（对照）|
|:--:|:---------|:--------:|:--------------:|:------------:|
| 3 | 2.00/0.002（顶）| 0.1875 | 2.00/0.000 | 0.240（冻结开始）|
| **4** | **0.81/0.258（回落）** | 0.1619 | 2.00/0.000 | 0.240 |
| 5 | 0.88/0.183 | 0.1500 | 2.00/0.000 | 0.240 |
| 6 | 0.88/0.337 | 0.1450 | 2.00/0.000 | 0.240 |

**两项判据修正**：
1. **v1 的病理不是"增益饱和"本身**，而是"增益饱和 **且** 无其他提亮路径 **且** 负载改道爆炸"的**系统级卡死**。v1.1 的 gain 在 ep3 同样到顶（暗中间态下 inter/gain_sup 目标=gt/暗→钳 2.0，早期与提亮同向——设计内的过渡态），但 brightness 锚在打开阶段门、si_cap 压住 s_illum——系统在演进
2. **观察点 (a) 兑现**：阶段变亮后（ep4）增益目标落到 2.0 以下，**gain 从饱和区回落至 0.81 并发展出空间结构**（std 0.26→0.34）——sigmoid 尾部的残差梯度（ep3 std=0.002 未完全死亡）足以在锚点配合下恢复。观察点 (b) 兑现：pix 持续下降 0.385→0.145@ep6（v1 冻结 0.240；S1 旧损失参考 ~0.120@ep5）

**遗留观察**：s_illum 均值降到 0.087±1.00（ep6）——远低于 S1 的 0.85-0.95 区间，提亮负载主要改道 TCC 曲线与阶段门。非病理（pix 正常下降），但属行为域偏移，phase1_5/phase2 期间持续关注。**剩余验收点：ep10 val ≥ 16.70（S1 对照）**。

失败 run 存档于 `outputs/sdsd_f11_simple/`（v1）；v1.1 运行于 `outputs/sdsd_f11_simple2/`（keepalive 目标 40ep）。
...
### 7.5 S1.5 双 run 对照分析（v1 失败 → v1.1 恢复，2026-09-11）

同主干（T-BC1b + P1/P2/P3）、同配置、同 seed，唯一变量 = 损失形态：

| 观测量 | v1（纯 5 项）| v1.1（8 项含锚）| S1（15 项 UW，参照）|
|--------|-------------|----------------|-------------------|
| gain @ep4-6 | 2.00/0.000 **永久冻结** | **0.81→0.88/0.18-0.34 恢复+空间结构** | 1.62→1.26 振荡 |
| s_illum 均值 | 1.95（爆炸改道）| 0.39→0.09（被护栏压低）| 0.85-0.95 |
| train pix @ep5 | 0.240 卡死 | **0.150 持续下降** | 0.120 |
| ep10 val | 12.54 | 待出 | 16.70 |

三条分析结论：

1. **锚点的本质是"分工保证"**：brightness/inter/gain_sup 三锚不直接优化输出质量，它们强制**提亮负载在阶段门→增益→曲线→s_illum 之间按架构设计分工**。删掉后系统寻找的"最优解"是病态的（单一变量过载饱和），而非更优
2. **概念模型证据的适用边界**：单 pix+ssim 的 +1.75dB 证据只在"无分阶变量的端到端形态"下成立。任何带分阶物理量的架构，其损失 = 核心项 + 该物理量的锚点——v1 跨过边界付出了 4.16dB 的代价，v1.1 以 3 个锚点（8 项 vs 15 项）回到边界内
3. **S1.5 v1.1 与 S1 的 pix 差距（0.150 vs 0.120 @ep5）**：来源待判——可能是 s_illum 低均值行为域偏移（提亮改道曲线）或锚点初期的过渡成本。ep10 val 是分晓点：≥16.70 即确认简化损失无损
...
| 风险 | 缓解/回退 |
|------|----------|
| V1 未达标（pair45 < 18）| **已触发（13.51）**。X3 消融已执行（2026-09-13）：luckiness/disp_field/s_illum 三开关全部惰性或噪声级（+0.00/+0.00/+0.05）→ 归因从"接线"转向"训练动力学"，A2（损失简化移除分支监督）为首要嫌疑；下一步 S1 旧损失续训至 ep30 定罪。详见 RESULTS.md §7.5 |
