...
> 日期：2026-07-17 (训练于 07-20 ep22 终止)  
> 基线：Flight7.2+WFR (PSNR=17.10@ep80, dpe_si=0.893/0.063)  
> 目标：解决 DPE s_illum 饱和 + 提升 WKV 空间建模能力  
> **结果：提前终止 — DPE 饱和未解决，ep20 PSNR=14.68 (↓2.42 vs baseline)，迁移到 Flight9**

### 最终训练指标 (ep22)

| 指标 | 值 | 判定 |
|------|-----|:---:|
| PSNR (ep10/20) | 16.33 / 14.68 | ep10 -0.86 vs F7.2 |
| dpe_si (mean/std) | 0.920 / **0.000** | 完全饱和 ✗ |
| gain (mean/std) | 1.57 / 0.512 | 偏高但可用 |
| gn (gamma) | **0.049** (5×) | 暴走 ✗ |
| gm (gamma) | 0.027 (2.7×) | 适中 ✓ |
| LPIPS | 0.343 | 改善 ✓ |
| Phase 1.5 dip | -1.65 dB | 稍好 vs -2.06 ✓ |
| frame_cache | 64帧 LRU | 工作正常 ✓ |
| batched encoder | B*T 一次通过 | 无退化 ✓ |
