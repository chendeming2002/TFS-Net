...
## 九、Golf-R2 实验结果（2026-09-20 更新）

> 训练：Golf-R1 停止（L_div bug），从零重训 Golf-R2（60ep）

### 9.1 最终验证结果

| 指标 | ep40 | **ep60（终判）** | vs 成功标准 |
|------|:---:|:---:|:---:|
| Val PSNR | 18.868 | **20.140** | ✅ ≥19.7 |
| Val SSIM | 0.7264 | **0.7405** | |
| Val LPIPS | 0.3292 | **0.3143** | |
| pair45 PSNR（前30帧） | **16.063** | - | ✅ ≥15.0 |

**结论**：三项成功标准全部达标，ep60 突破 20 dB 大关。

### 9.2 修复验证

| 修复项 | 验证方式 | 结果 |
|-------|---------|------|
| G1 resize-conv 棋盘格修复 | 自相关分析 + 视觉 | ✅ 2px 棋盘格消失 |
| G3 余弦窗口缝线修复 | 1080p 推理帧视觉检查 | ✅ 224px 分格消失 |
| G2 F1 skip | pair45 +1.69 dB vs Foxtrot | ✅ 细节改善 |
| L_div 修正 | 训练中 div -0.117→-0.356 单调 | ✅ 方向正确 |

### 9.3 工程变更

- L_chess 删除（恒为零死代码，原理错位）
- L_div 符号修复（Golf-R1→Golf-R2 重训）
- golf_r2.yaml epochs: 60（延长 20ep，验证还有提升空间的判断）

### 9.4 遗留问题与下一版本方向

详见 `docs/v6/Golf-R2-analysis.md` §四（仍待解决的问题）。
