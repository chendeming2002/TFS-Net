...
### 四-A：v6.5 实证验证（2026-06-28）

v6.4 的 DAT 能量占比 77% 超过 60% 阈值，但 v6.5 仍选择实施纯 RWKV 替代 DAT，以验证理论可行性。

**v6.5 训练结果** (SDSD indoor, 30 epoch, 1.17M 参数):

| Ep | PSNR | SSIM | 对比 v5.9.2 (DAT) |
|---|---|---|---|
| 5 | 19.74 | 0.7620 | +0.13 dB |
| 20 | 20.07 | 0.7676 | — |
| 25 | 20.32 | 0.7689 | — |
| **30** | **20.36** | 0.7684 | **-0.03 dB** |

**结论**: v6.5 PSNR=20.36 仅比 v5.9.2 (DAT, 20.39) 差 0.03 dB，**基本追平**。证明：
1. ✅ 多尺度双向 RWKV 可等效替代可变形采样对齐
2. ✅ 纯 RWKV 不需要 grid_sample 式的几何 warping
3. ✅ 参数量 1.17M (vs DAT 版的 1.14M, +2.6%)

**v6 Bravo 进行中**: 在 v6.5 基础上叠加 DWT-LFF 分裂 + phase_conf + 损失调整。
