...
| A2 (LN+IH only) | ❌ 回退 | 2026-06-22 | batch=3 crop=224 v5.5参数, ep5=18.56 (-0.53dB)。训练损失最低(0.260)但验证最差 → **LN导致过拟合** |
| **结论** | **全部 P0+P1 架构改进回退** | 2026-06-22 | **v5.5 无归一化 Conv 主干在 SDSD indoor(小数据集)上泛化更好。LN 在浅 Conv(2-4层/模块)中不改善泛化, 反而加速过拟合。NAFNet 用 LN 有效是因 30+ 层深度堆叠, TFS-Net 不具备。** |
