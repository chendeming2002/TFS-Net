...
class MCPN(nn.Module):
    def __init__(self, channels=64, window_size=8, num_frames=5):
        ...
        nn.init.zeros_(self.motion_estimator[-2].weight)
        nn.init.zeros_(self.motion_estimator[-2].bias)
        ...
        # P0: motion refinement — single 3×3 only (1×1 has no spatial meaning for motion)
    ...
    def reset_startup(self):
        ...
        self.gamma.data.fill_(0.01)  # Flight7.1: restored to Flight6-verified value
    ...
    def forward(self, F_aligned_list, center_idx, sigma_t_clean=None,
        ...
        motion_delta = self.motion_refine(torch.cat([f_center, f_omega_aligned], dim=1))
