...
class TCA_RWKV(nn.Module):
    ...
    def __init__(self, channels: int = 64, tca_channels: int = 128,
        ...
        # R3-C: F3 全局描述子 → FiLM 调制三路查询
        # F3 (B,128,H/4,W/4) → global avg pool → MLP → 3×channels 调制参数
        # 零初始化：训练初期不调制，让模型自然学习退化感知
        if use_f3_film:
            self.f3_film = nn.Linear(f3_channels, 3 * channels, bias=True)
    ...
    def forward(self, center_feat: torch.Tensor, feats_seq: torch.Tensor,
        ...
        # R3-C: F3 FiLM 退化感知查询调制
        # F3 全局描述子 → sigmoid gate → 调制三路查询的空间注意力强度
        if self.use_f3_film and f3_ctx is not None:
            film_params = self.f3_film(f3_ctx)           # (B, 3*C)
            scale_N, scale_L, scale_M = film_params.chunk(3, dim=-1)
            # sigmoid 保证 gate ∈ (0,1)，零初始化时 gate=0.5 (中性)
            gate_N = torch.sigmoid(scale_N).unsqueeze(-1).unsqueeze(-1)
            gate_L = torch.sigmoid(scale_L).unsqueeze(-1).unsqueeze(-1)
            gate_M = torch.sigmoid(scale_M).unsqueeze(-1).unsqueeze(-1)
            Q_N = Q_N * gate_N
            Q_L = Q_L * gate_L
            Q_M = Q_M * gate_M
