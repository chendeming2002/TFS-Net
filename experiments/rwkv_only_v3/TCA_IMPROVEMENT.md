...
## 6. 后续实验计划（对接三分支，2026-09-10 定稿）

> 完整论证与排序依据见 `RESULTS.md` §6 与 `docs/v6/TSDR-framework-716.md` M1/M2 节。
> 本文件保留机制层设计（T-A/T-B/T-C 的原理与实现），实验计划以 RESULTS.md §6 为准，此处仅列对应关系。

### T-BC 系列与三分支对接项的映射

| 计划项 | 机制来源 | 对接目标 |
|--------|---------|---------|
| P1 disp_field | T-B（窗口 softmax soft-argmax 副产物）| MCPN 运动幅度门 |
| P2 warped_list + conf_map | T-B 接口预留 | NDPN 平均材料 + conf_proj 替换 |
| P3 luckiness 残差门 | CDVD-TSP（新增，与 T-B 互补：先验 vs 后验）| NDPN/MCPN 聚合权重 |
| P4 s_edge + 自洽 loss | DRWKV GER（新增）| DPE 防塌 + 双分支边界判据 |
| P5 LAN 亮度状态调制 | URWKV LAN（新增，ISPN 时间锚定）| ISPN refine 注入 |
| P7 disp_field 外部监督 | STCD/StableLLVE（训练期）| 位移场质量 |

### 执行状态

- T-A（算子修复）：✅ 已落地共享模块
- T-BC1（窗口对齐 v1）：✅ 完成——19.69@20，conf 死锁诊断
- T-BC1b（bootstrap=bias + 逐帧 softmax）：✅ 完成——**19.78@20 / 19.99@40**，conf 激活至 0.70，pair45 +0.65dB
- **T-BC1b 已转正为下一完整版本主干**；后续按 RESULTS.md §6 的 S1→S2→S3 推进（S1 = P1/P2/P3 集成，验收 pair45 ≥ 18）
