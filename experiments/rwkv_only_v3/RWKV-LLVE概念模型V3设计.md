...
## 7. 实验执行结果（2026-09-04 更新）

本文所有预测已执行完毕，结果与两处关键预测不符：

1. **motion_map 确认完全退化**（std=0.0000，全局常数）——1.1 节的质疑成立
2. **方案 3（sigmoid 移除）实测有害 -0.29dB**（ablation A），第 5/6 节"+0.5dB free lunch"预测证伪
3. 方案 2A+4 疑似 +0.26dB（由 v3 与 A 的差值反推），验证实验 BC 进行中

完整数据、归因分析和工程结论见 **[RESULTS.md](./RESULTS.md)**。